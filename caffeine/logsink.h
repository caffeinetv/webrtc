...
namespace {
    ...
    constexpr bool isEnumEqual(enum caff_log_severity caff, rtc::LoggingSeverity rtc)
    {
        return caff == static_cast<caff_log_severity>(rtc);
    }

    static_assert(isEnumEqual(CAFF_LOG_SENSITIVE, rtc::LoggingSeverity::LS_SENSITIVE),
...
namespace caff {
    ...
    public:
        LogSink(caff_log_callback_t cb)
            : callback(cb)
        { }
